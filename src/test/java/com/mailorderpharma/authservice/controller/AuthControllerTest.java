...
class AuthControllerTest {
	...
	@Test
	 void loginTest() {
		...
		UserData user = new UserData("priya", "priya",null,null);
		UserDetails loadUserByUsername = custdetailservice.loadUserByUsername("priya");
		UserDetails value = new User(user.getUserid(), user.getUpassword(), new ArrayList<>());
		when(custdetailservice.loadUserByUsername("priya")).thenReturn(value);
	...
	@Test
	 void loginTestFailed() {
		...
		UserData user = new UserData("priya", "priya",null,null);
		UserDetails loadUserByUsername = custdetailservice.loadUserByUsername("priya");
		UserDetails value = new User(user.getUserid(), "priya11", new ArrayList<>());
		when(custdetailservice.loadUserByUsername("priya")).thenReturn(value);
		when(jwtutil.generateToken(loadUserByUsername)).thenReturn("token");
		ResponseEntity<?> login = authController.login(user);
	...
	@Test
	 void validateTestValidtoken() {
		...
		when(jwtutil.extractUsername("token")).thenReturn("priya");
		UserData user1 = new UserData("priya", "priya", "priya",null);
		Optional<UserData> data = Optional.of(user1);
		when(userservice.findById("priya")).thenReturn(data);
